...
SERVICE_FILE="$ROOT_DIR/deploy/kubesafe.service"

VERSION_FILE="$SERVER_DIR/cmd/kubesafe/VERSION"
...
PACKAGE_STEM="kubesafe_${VERSION}_${GOOS}_${GOARCH}"
...
(
  cd "$WEB_DIR"
  KUBESAFE_WEB_OUT_DIR=../server/internal/web/dist/app npm run build
)
...
(
  ...
  GOOS="$GOOS" GOARCH="$GOARCH" go build \
    ...
    -o "$PACKAGE_DIR/kubesafe" \
    ./cmd/kubesafe
)

cp "$SERVICE_FILE" "$PACKAGE_DIR/kubesafe.service"
