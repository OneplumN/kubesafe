# kubesafe 开发与实验集群操作指南

## 1. 文档范围

- 本文档用于说明 `kubesafe` 当前对接的本地实验集群信息、日常运维入口以及本机开发联调方式。
- 当前实验环境对应仓库：`/Users/zhangya/workspace/k8s-dev`
- 当前产品仓库：`git@github.com:OneplumN/kubesafe.git`
...
```bash
kubectl create serviceaccount kubesafe-dev -n kube-system
kubectl create clusterrolebinding kubesafe-dev \
  --clusterrole=cluster-admin \
  --serviceaccount=kube-system:kubesafe-dev
kubectl create token kubesafe-dev -n kube-system
```
