...
import (
	...
	"github.com/OneplumN/kubesafe/server/internal/ptyx"
	"github.com/OneplumN/kubesafe/server/internal/response"
...
func newRouter(
	...
	api := router.Group("/api/v1")
	{
		...
		authorized.Use(clusterServiceMiddleware(clusterFactory))
		{
			...
			authorized.POST("/manifests/batch", func(c *gin.Context) {
				var req yamlUpdateRequest
				if err := c.ShouldBindJSON(&req); err != nil {
					c.JSON(http.StatusBadRequest, response.Failure("INVALID_YAML_BATCH_CREATE_REQUEST", "请求体格式不正确"))
					return
				}

				result, err := mustClusterService(c).CreateManifestBatchYAML(
					c.Request.Context(),
					req.Content,
				)
				if err != nil {
					respondWithClusterError(c, "CREATE_MANIFEST_BATCH_FAILED", err)
					return
				}
			...
			authorized.POST("/manifests/apply", func(c *gin.Context) {
				var req yamlUpdateRequest
				if err := c.ShouldBindJSON(&req); err != nil {
					c.JSON(http.StatusBadRequest, response.Failure("INVALID_YAML_APPLY_REQUEST", "请求体格式不正确"))
					return
				}

				result, err := mustClusterService(c).ApplyManifestBatchYAML(
					c.Request.Context(),
					req.Content,
				)
				if err != nil {
					respondWithClusterError(c, "APPLY_MANIFEST_FAILED", err)
					return
				}
			...
			authorized.POST("/simulation/reset", func(c *gin.Context) {
				namespace := strings.TrimSpace(c.Query("namespace"))
				result, err := mustClusterService(c).ResetSimulationNamespace(
					c.Request.Context(),
					namespace,
				)
				if err != nil {
					respondWithClusterError(c, "RESET_SIMULATION_FAILED", err)
					return
				}
			...
			authorized.GET("/simulation/scenarios", func(c *gin.Context) {
				items, err := mustClusterService(c).ListSimulationScenarios(c.Request.Context())
				if err != nil {
					respondWithClusterError(c, "LIST_SIMULATION_SCENARIOS_FAILED", err)
					return
				}
			...
			authorized.POST("/simulation/scenarios/:id/reset", func(c *gin.Context) {
				result, err := mustClusterService(c).ResetSimulationScenario(
					c.Request.Context(),
					c.Param("id"),
				)
				if err != nil {
					respondWithClusterError(c, "RESET_SIMULATION_SCENARIO_FAILED", err)
					return
				}
