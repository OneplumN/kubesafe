...
type Factory struct {
	...
	mode       string
	simKube    kubernetes.Interface
	simMetrics metricsclient.Interface
}
...
func NewFactory(configPath string, mode string) (*Factory, error) {
	if strings.TrimSpace(mode) == "simulation" {
		return NewSimulationFactory(), nil
	}
...
func (f *Factory) NewClientForToken(token string) (*Client, error) {
	...
	if f.mode == "simulation" {
		return &Client{
			Kubernetes:  f.simKube,
			Metrics:     f.simMetrics,
			RESTConfig:  &rest.Config{Host: "https://kubesafe-simulation.local"},
