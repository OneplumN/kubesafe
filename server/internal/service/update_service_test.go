...
func TestDownloadFileUsesCallerContextWithoutInjectedDeadline(t *testing.T) {
	...
	service := NewUpdateService(buildinfo.Info{}, config.UpdateConfig{}, false)
	tempDir := t.TempDir()
	destPath := tempDir + "/kubesafe.tar.gz"
	...
	if err := service.downloadFile(context.Background(), "https://github.com/example/project/releases/download/v1.0.0/kubesafe.tar.gz", destPath); err != nil {
...
func TestCheckForActorMarksPendingRestartFromInstalledVersion(t *testing.T) {
	...
	tempDir := t.TempDir()
	managedPath := filepath.Join(tempDir, "kubesafe")
...
func TestPerformUpdateUsesConfiguredTargetPathAndStagesRestart(t *testing.T) {
	...
	archiveName := "kubesafe_0.1.4_" + runtime.GOOS + "_" + runtime.GOARCH + ".tar.gz"
...
func TestNormalizeManagedBinaryPathStripsLegacyBackupSuffixes(t *testing.T) {
	...
	actual := normalizeManagedBinaryPath("/opt/kubesafe/kubesafe.backup.backup")
	if actual != "/opt/kubesafe/kubesafe" {
		t.Fatalf("normalizeManagedBinaryPath() = %q, want %q", actual, "/opt/kubesafe/kubesafe")
	}
}
...
func writeVersionScript(t *testing.T, path string, version string) {
	...
	content := fmt.Sprintf(
		"#!/bin/sh\nif [ \"$1\" = \"-version\" ]; then\n  echo 'kubesafe %s (commit: test, built: test, type: release)'\n  exit 0\nfi\nexit 0\n",
		version,
	)
