...
const (
	...
	defaultUserAgent    = "kubesafe-update-client"
)
...
func (s *UpdateService) PerformUpdate(ctx context.Context, actor string) (*UpdateActionResult, error) {
	...
	tempDir, err := os.MkdirTemp(exeDir, ".kubesafe-update-*")
	if err != nil {
	...
	newBinaryPath := filepath.Join(tempDir, "kubesafe")
...
func (s *UpdateService) fetchLatestStableReleaseFromList(ctx context.Context, repo string) (*githubRelease, error) {
	...
	if err != nil {
		...
		if errors.As(err, &apiErr) && apiErr.StatusCode == http.StatusNotFound {
			return nil, fmt.Errorf(
				"cannot access GitHub releases for %s; verify KUBESAFE_UPDATE_REPOSITORY or set KUBESAFE_UPDATE_GITHUB_TOKEN if the repository is private",
				repo,
			)
		}
	...
	if err != nil {
		if hasUsablePrerelease(releases) {
			return nil, fmt.Errorf(
				"no stable release is published for %s yet; set KUBESAFE_UPDATE_ALLOW_PRERELEASES=true to use prerelease builds",
				repo,
			)
		}
...
func extractBinaryFromArchive(archivePath string, destPath string) error {
	...
	for {
		...
		if filepath.Base(header.Name) != "kubesafe" {
			continue
		}
	...
	return fmt.Errorf("binary kubesafe not found in archive")
}
...
func parseVersionOutput(output []byte) (string, error) {
	...
	if len(fields) < 2 || !strings.EqualFold(fields[0], "kubesafe") {
...
func (s *UpdateService) expectedArchiveName(version string) string {
	return fmt.Sprintf("kubesafe_%s_%s_%s.tar.gz", version, runtime.GOOS, runtime.GOARCH)
}
