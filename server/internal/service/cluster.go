...
import (
	...
	"github.com/OneplumN/kubesafe/server/internal/jsonx"
...
type ManifestBatchResult struct {
	Count     int                               `json:"count"`
	Results   jsonx.Slice[WorkloadActionResult] `json:"results"`
	Message   string                            `json:"message"`
	Timestamp string                            `json:"timestamp"`
}

type SimulationResetResult struct {
	Namespace string              `json:"namespace"`
...
func (s *ClusterService) simulationMode() bool {
	return s != nil && s.client != nil && s.client.AuthMode == "simulation"
}
...
func (s *ClusterService) ListPods(ctx context.Context, namespace string) ([]PodItem, error) {
	...
	if s.simulationMode() {
		s.fillSimulationPodMetrics(items.Items, metricsByPod)
	}
...
func (s *ClusterService) DeletePod(
	...
	if s.simulationMode() {
		...
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("get pod %s/%s before delete: %w", namespace, name, err)
		}
		ownerDeployment = s.simulationPodDeploymentOwner(ctx, pod)
	}
	...
	if ownerDeployment != "" {
		if err := s.reconcileSimulationDeployment(ctx, namespace, ownerDeployment); err != nil {
			return WorkloadActionResult{}, err
		}
	}
...
func (s *ClusterService) DeleteResource(
	...
	if s.simulationMode() {
		result, err := s.deleteSimulationResource(ctx, kind, namespace, name)
		if err != nil {
			return WorkloadActionResult{}, err
		}
		return result, nil
...
func (s *ClusterService) GetPodLogs(
	...
	if s.simulationMode() {
		return PodLogResult{
			...
			Name:        name,
			Container:   container,
			Content:     simulationPodLogs(*pod, container, tailLines),
...
func (s *ClusterService) GetPodDescribe(
	...
	if s.simulationMode() {
		...
		events, err := s.ListPodEvents(ctx, namespace, name)
		if err != nil {
			return ResourceTextResult{}, err
		}
		return ResourceTextResult{
			Namespace:   namespace,
			Name:        name,
			Content:     simulationPodDescribe(*pod, events),
			GeneratedAt: time.Now().Format("2006-01-02 15:04:05"),
		}, nil
	}
...
func (s *ClusterService) GetResourceYAML(
	...
	if s.simulationMode() {
		return s.getSimulationResourceYAML(ctx, kind, namespace, name)
	}
...
func (s *ClusterService) ApplyResourceYAML(
	...
	if s.simulationMode() {
		return s.applySimulationResourceYAML(ctx, kind, namespace, name, content)
	}
...
func (s *ClusterService) CreateManifestYAML(
	...
	if s.simulationMode() {
		return s.createSimulationManifest(ctx, kind, namespace, name, content)
	}
...
func (s *ClusterService) CreateManifestBatchYAML(
	ctx context.Context,
	content string,
) (ManifestBatchResult, error) {
	documents, err := splitManifestYAMLDocuments(content)
	if err != nil {
		return ManifestBatchResult{}, err
	}
	if len(documents) == 0 {
		return ManifestBatchResult{}, newValidationError("yaml content is required")
	}

	results := make([]WorkloadActionResult, 0, len(documents))
	for index, document := range documents {
		result, err := s.CreateManifestYAML(ctx, document)
		if err != nil {
			return ManifestBatchResult{}, fmt.Errorf("create manifest document %d: %w", index+1, err)
		}
		results = append(results, result)
	}

	if s.simulationMode() {
		if err := s.reconcileSimulationNamespaces(ctx, results); err != nil {
			return ManifestBatchResult{}, err
		}
	}
	...
	return ManifestBatchResult{
		Count:     len(results),
		Results:   jsonx.Slice[WorkloadActionResult](results),
		Message:   fmt.Sprintf("%d 个 Manifest 已创建", len(results)),
		Timestamp: time.Now().Format("2006-01-02 15:04:05"),
	}, nil
}

func (s *ClusterService) ApplyManifestBatchYAML(
	ctx context.Context,
	content string,
) (ManifestBatchResult, error) {
	documents, err := splitManifestYAMLDocuments(content)
	...
	if !s.simulationMode() {
		if _, err := s.runKubectlCommand(ctx, bytes.NewBufferString(strings.TrimSpace(content)), "apply", "-f", "-"); err != nil {
			return ManifestBatchResult{}, fmt.Errorf("apply manifest yaml: %w", err)
		}
		results := make([]WorkloadActionResult, 0, len(documents))
		for index, document := range documents {
			kind, namespace, name, err := parseManifestIdentity(document)
			if err != nil {
				return ManifestBatchResult{}, fmt.Errorf("parse manifest document %d: %w", index+1, err)
			}
			results = append(results, WorkloadActionResult{
				Kind:      kind,
				Namespace: namespace,
				Name:      name,
				Operation: "apply",
				Message:   fmt.Sprintf("%s YAML 已应用", kind),
				Timestamp: time.Now().Format("2006-01-02 15:04:05"),
			})
		}
		return ManifestBatchResult{
			Count:     len(results),
			Results:   jsonx.Slice[WorkloadActionResult](results),
			Message:   fmt.Sprintf("%d 个 Manifest 已应用", len(results)),
			Timestamp: time.Now().Format("2006-01-02 15:04:05"),
		}, nil
	}
	...
	results := make([]WorkloadActionResult, 0, len(documents))
	for index, document := range documents {
		result, err := s.applySimulationManifest(ctx, document)
		if err != nil {
			return ManifestBatchResult{}, fmt.Errorf("apply manifest document %d: %w", index+1, err)
		}
		results = append(results, result)
	}
...
func (s *ClusterService) ResetSimulationNamespace(ctx context.Context, namespace string) (SimulationResetResult, error) {
	if !s.simulationMode() {
		return SimulationResetResult{}, newValidationError("simulation reset is only available in simulation mode")
	}
	...
	addDeleted := func(kind string, name string) {
		deleted = append(deleted, fmt.Sprintf("%s/%s", kind, name))
	}

	deployments, err := s.client.Kubernetes.AppsV1().Deployments(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return SimulationResetResult{}, fmt.Errorf("list deployments for simulation reset: %w", err)
	}
	for _, item := range deployments.Items {
		if !simulationLabResourceName(item.Name) {
			continue
		}
		if _, err := s.deleteSimulationResource(ctx, "Deployment", namespace, item.Name); err != nil {
			return SimulationResetResult{}, err
		}
		addDeleted("Deployment", item.Name)
	}

	services, err := s.client.Kubernetes.CoreV1().Services(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return SimulationResetResult{}, fmt.Errorf("list services for simulation reset: %w", err)
	}
	for _, item := range services.Items {
		if !simulationLabResourceName(item.Name) {
			continue
		}
		if err := s.client.Kubernetes.CoreV1().Services(namespace).Delete(ctx, item.Name, metav1.DeleteOptions{}); err != nil {
			return SimulationResetResult{}, fmt.Errorf("delete service %s/%s for simulation reset: %w", namespace, item.Name, err)
		}
		addDeleted("Service", item.Name)
	}

	endpoints, err := s.client.Kubernetes.CoreV1().Endpoints(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return SimulationResetResult{}, fmt.Errorf("list endpoints for simulation reset: %w", err)
	}
	for _, item := range endpoints.Items {
		if !simulationLabResourceName(item.Name) {
			continue
		}
		if err := s.client.Kubernetes.CoreV1().Endpoints(namespace).Delete(ctx, item.Name, metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
			return SimulationResetResult{}, fmt.Errorf("delete endpoints %s/%s for simulation reset: %w", namespace, item.Name, err)
		}
		addDeleted("Endpoints", item.Name)
	}

	configMaps, err := s.client.Kubernetes.CoreV1().ConfigMaps(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return SimulationResetResult{}, fmt.Errorf("list configmaps for simulation reset: %w", err)
	}
	for _, item := range configMaps.Items {
		if !simulationLabResourceName(item.Name) {
			continue
		}
		if err := s.client.Kubernetes.CoreV1().ConfigMaps(namespace).Delete(ctx, item.Name, metav1.DeleteOptions{}); err != nil {
			return SimulationResetResult{}, fmt.Errorf("delete configmap %s/%s for simulation reset: %w", namespace, item.Name, err)
		}
		addDeleted("ConfigMap", item.Name)
	}

	secrets, err := s.client.Kubernetes.CoreV1().Secrets(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return SimulationResetResult{}, fmt.Errorf("list secrets for simulation reset: %w", err)
	}
	for _, item := range secrets.Items {
		if !simulationLabResourceName(item.Name) {
			continue
		}
		if err := s.client.Kubernetes.CoreV1().Secrets(namespace).Delete(ctx, item.Name, metav1.DeleteOptions{}); err != nil {
			return SimulationResetResult{}, fmt.Errorf("delete secret %s/%s for simulation reset: %w", namespace, item.Name, err)
		}
		addDeleted("Secret", item.Name)
	}

	ingresses, err := s.client.Kubernetes.NetworkingV1().Ingresses(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return SimulationResetResult{}, fmt.Errorf("list ingresses for simulation reset: %w", err)
	}
	for _, item := range ingresses.Items {
		if !simulationLabResourceName(item.Name) {
			continue
		}
		if err := s.client.Kubernetes.NetworkingV1().Ingresses(namespace).Delete(ctx, item.Name, metav1.DeleteOptions{}); err != nil {
			return SimulationResetResult{}, fmt.Errorf("delete ingress %s/%s for simulation reset: %w", namespace, item.Name, err)
		}
		addDeleted("Ingress", item.Name)
	}

	claims, err := s.client.Kubernetes.CoreV1().PersistentVolumeClaims(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return SimulationResetResult{}, fmt.Errorf("list pvcs for simulation reset: %w", err)
	}
	for _, item := range claims.Items {
		if !simulationLabResourceName(item.Name) {
			continue
		}
		if err := s.client.Kubernetes.CoreV1().PersistentVolumeClaims(namespace).Delete(ctx, item.Name, metav1.DeleteOptions{}); err != nil {
			return SimulationResetResult{}, fmt.Errorf("delete pvc %s/%s for simulation reset: %w", namespace, item.Name, err)
		}
		addDeleted("PersistentVolumeClaim", item.Name)
	}

	pods, err := s.client.Kubernetes.CoreV1().Pods(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return SimulationResetResult{}, fmt.Errorf("list pods for simulation reset: %w", err)
	}
	for _, item := range pods.Items {
		if !simulationLabResourceName(item.Name) {
			continue
		}
		if err := s.client.Kubernetes.CoreV1().Pods(namespace).Delete(ctx, item.Name, metav1.DeleteOptions{}); err != nil {
			return SimulationResetResult{}, fmt.Errorf("delete pod %s/%s for simulation reset: %w", namespace, item.Name, err)
		}
		addDeleted("Pod", item.Name)
	}
	...
	return SimulationResetResult{
		Namespace: namespace,
		Deleted:   len(deleted),
		Resources: jsonx.Slice[string](deleted),
		Message:   fmt.Sprintf("已清理 %s 命名空间内 %d 个实验资源", namespace, len(deleted)),
		Timestamp: time.Now().Format("2006-01-02 15:04:05"),
	}, nil
}

func simulationLabResourceName(name string) bool {
	name = strings.TrimSpace(name)
	return strings.HasPrefix(name, "yaml-") || strings.HasPrefix(name, "rt-")
}

func parseManifestIdentity(content string) (string, string, string, error) {
	var manifest resourceManifestIdentity
	if err := yaml.Unmarshal([]byte(content), &manifest); err != nil {
		return "", "", "", newValidationError("parse manifest yaml: %v", err)
	}
	...
	if apiVersion == "" {
		return "", "", "", newValidationError("yaml apiVersion is required")
	}
	if kind == "" {
		return "", "", "", newValidationError("yaml kind is required")
	}
	if name == "" {
		return "", "", "", newValidationError("yaml metadata.name is required")
	}
	if namespace == "" && simulationKindIsNamespaced(kind) {
		namespace = "default"
	}
...
func splitManifestYAMLDocuments(content string) ([]string, error) {
	content = strings.TrimSpace(content)
	if content == "" {
		return nil, nil
	}
	...
	for {
		...
		if err := decoder.Decode(&node); err != nil {
			...
			return nil, newValidationError("parse manifest yaml: %v", err)
		}
		if len(node.Content) == 0 {
			continue
		}
		raw, err := yamlv3.Marshal(node.Content[0])
		if err != nil {
			return nil, fmt.Errorf("marshal manifest yaml document: %w", err)
		}
...
func (s *ClusterService) createSimulationManifest(
	ctx context.Context,
	kind string,
	namespace string,
	name string,
	...
	case "Deployment":
		...
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
			return WorkloadActionResult{}, newValidationError("parse deployment yaml: %v", err)
		}
		item.Namespace = defaultString(item.Namespace, namespace)
		item.Name = defaultString(item.Name, name)
		if item.Spec.Replicas == nil {
			item.Spec.Replicas = int32Ptr(1)
		}
		if item.Spec.Selector == nil || len(item.Spec.Selector.MatchLabels) == 0 {
			return WorkloadActionResult{}, newValidationError("deployment spec.selector.matchLabels is required")
		}
		if len(item.Spec.Template.Labels) == 0 {
			item.Spec.Template.Labels = item.Spec.Selector.MatchLabels
		}
		created, err := s.client.Kubernetes.AppsV1().Deployments(item.Namespace).Create(ctx, &item, metav1.CreateOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("create deployment %s/%s from yaml: %w", item.Namespace, item.Name, err)
		}
		if err := s.reconcileSimulationDeployment(ctx, created.Namespace, created.Name); err != nil {
			return WorkloadActionResult{}, err
		}
	...
	case "Service":
		...
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
			return WorkloadActionResult{}, newValidationError("parse service yaml: %v", err)
		}
		item.Namespace = defaultString(item.Namespace, namespace)
		item.Name = defaultString(item.Name, name)
		if item.Spec.ClusterIP == "" {
			item.Spec.ClusterIP = fmt.Sprintf("10.96.%d.%d", len(item.Name)+20, len(item.Namespace)+20)
		}
		if _, err := s.client.Kubernetes.CoreV1().Services(item.Namespace).Create(ctx, &item, metav1.CreateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("create service %s/%s from yaml: %w", item.Namespace, item.Name, err)
		}
		if err := s.reconcileSimulationServiceEndpoints(ctx, item.Namespace, item.Name); err != nil {
			return WorkloadActionResult{}, err
		}
		namespace = item.Namespace
	case "ConfigMap":
		var item corev1.ConfigMap
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
			return WorkloadActionResult{}, newValidationError("parse configmap yaml: %v", err)
		}
		item.Namespace = defaultString(item.Namespace, namespace)
		item.Name = defaultString(item.Name, name)
		if _, err := s.client.Kubernetes.CoreV1().ConfigMaps(item.Namespace).Create(ctx, &item, metav1.CreateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("create configmap %s/%s from yaml: %w", item.Namespace, item.Name, err)
		}
		namespace = item.Namespace
	case "Secret":
		var item corev1.Secret
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
			return WorkloadActionResult{}, newValidationError("parse secret yaml: %v", err)
		}
		item.Namespace = defaultString(item.Namespace, namespace)
		item.Name = defaultString(item.Name, name)
		if item.Type == "" {
		...
		if _, err := s.client.Kubernetes.CoreV1().Secrets(item.Namespace).Create(ctx, &item, metav1.CreateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("create secret %s/%s from yaml: %w", item.Namespace, item.Name, err)
		}
		namespace = item.Namespace
	case "Ingress":
		var item networkingv1.Ingress
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
			return WorkloadActionResult{}, newValidationError("parse ingress yaml: %v", err)
		}
		item.Namespace = defaultString(item.Namespace, namespace)
		item.Name = defaultString(item.Name, name)
		if _, err := s.client.Kubernetes.NetworkingV1().Ingresses(item.Namespace).Create(ctx, &item, metav1.CreateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("create ingress %s/%s from yaml: %w", item.Namespace, item.Name, err)
		}
		namespace = item.Namespace
	case "PersistentVolumeClaim":
		var item corev1.PersistentVolumeClaim
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
			return WorkloadActionResult{}, newValidationError("parse persistentvolumeclaim yaml: %v", err)
		}
		item.Namespace = defaultString(item.Namespace, namespace)
		item.Name = defaultString(item.Name, name)
		normalizeSimulationPVCStatus(&item)
		if _, err := s.client.Kubernetes.CoreV1().PersistentVolumeClaims(item.Namespace).Create(ctx, &item, metav1.CreateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("create persistentvolumeclaim %s/%s from yaml: %w", item.Namespace, item.Name, err)
		}
		namespace = item.Namespace
	default:
		return WorkloadActionResult{}, newValidationError("%s create is not simulated yet", kind)
	}
...
func (s *ClusterService) applySimulationManifest(ctx context.Context, content string) (WorkloadActionResult, error) {
	kind, namespace, name, err := parseManifestIdentity(content)
	if err != nil {
		return WorkloadActionResult{}, err
	}

	exists, err := s.simulationResourceExists(ctx, kind, namespace, name)
	if err != nil {
		return WorkloadActionResult{}, err
	}
	if exists {
...
func (s *ClusterService) simulationResourceExists(ctx context.Context, kind string, namespace string, name string) (bool, error) {
	switch strings.ToLower(kind) {
	case "deployment":
		_, err := s.client.Kubernetes.AppsV1().Deployments(namespace).Get(ctx, name, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			return false, nil
		}
		return err == nil, err
	case "service":
		_, err := s.client.Kubernetes.CoreV1().Services(namespace).Get(ctx, name, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			return false, nil
		}
		return err == nil, err
	case "configmap":
		_, err := s.client.Kubernetes.CoreV1().ConfigMaps(namespace).Get(ctx, name, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			return false, nil
		}
		return err == nil, err
	case "secret":
		_, err := s.client.Kubernetes.CoreV1().Secrets(namespace).Get(ctx, name, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			return false, nil
		}
		return err == nil, err
	case "ingress":
		_, err := s.client.Kubernetes.NetworkingV1().Ingresses(namespace).Get(ctx, name, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			return false, nil
		}
		return err == nil, err
	case "persistentvolumeclaim":
		_, err := s.client.Kubernetes.CoreV1().PersistentVolumeClaims(namespace).Get(ctx, name, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			return false, nil
		}
		return err == nil, err
	default:
		return false, newValidationError("%s apply is not simulated yet", kind)
	}
}

func simulationKindIsNamespaced(kind string) bool {
	switch kind {
	case "Deployment", "Service", "ConfigMap", "Secret", "Ingress", "PersistentVolumeClaim":
		return true
	default:
		return false
	}
...
func (s *ClusterService) reconcileSimulationNamespaces(ctx context.Context, results []WorkloadActionResult) error {
	...
	for namespace := range namespaces {
		deployments, err := s.client.Kubernetes.AppsV1().Deployments(namespace).List(ctx, metav1.ListOptions{})
		if err != nil {
			return fmt.Errorf("list simulation deployments for namespace reconcile %s: %w", namespace, err)
		}
		for _, deployment := range deployments.Items {
			if err := s.reconcileSimulationDeployment(ctx, namespace, deployment.Name); err != nil {
				return err
			}
		}
		if err := s.reconcileSimulationServiceEndpoints(ctx, namespace, ""); err != nil {
			return err
		}
	}
...
func (s *ClusterService) getSimulationResourceYAML(ctx context.Context, kind string, namespace string, name string) (ResourceTextResult, error) {
	var object any
	switch strings.ToLower(kind) {
	case "pod":
		item, err := s.client.Kubernetes.CoreV1().Pods(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return ResourceTextResult{}, fmt.Errorf("get simulation pod %s/%s yaml: %w", namespace, name, err)
		}
		item.APIVersion = "v1"
		item.Kind = "Pod"
		object = item
	case "deployment":
		...
		if err != nil {
			return ResourceTextResult{}, fmt.Errorf("get simulation deployment %s/%s yaml: %w", namespace, name, err)
		}
		item.APIVersion = "apps/v1"
		item.Kind = "Deployment"
		object = item
	case "service":
		item, err := s.client.Kubernetes.CoreV1().Services(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return ResourceTextResult{}, fmt.Errorf("get simulation service %s/%s yaml: %w", namespace, name, err)
		}
		item.APIVersion = "v1"
		item.Kind = "Service"
		object = item
	case "endpoints":
		if err := s.reconcileSimulationServiceEndpoints(ctx, namespace, name); err != nil {
			return ResourceTextResult{}, err
		}
		item, err := s.client.Kubernetes.CoreV1().Endpoints(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return ResourceTextResult{}, fmt.Errorf("get simulation endpoints %s/%s yaml: %w", namespace, name, err)
		}
		item.APIVersion = "v1"
		item.Kind = "Endpoints"
		object = item
	case "persistentvolumeclaim":
		item, err := s.client.Kubernetes.CoreV1().PersistentVolumeClaims(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return ResourceTextResult{}, fmt.Errorf("get simulation persistentvolumeclaim %s/%s yaml: %w", namespace, name, err)
		}
		item.APIVersion = "v1"
		item.Kind = "PersistentVolumeClaim"
		object = item
	default:
		return ResourceTextResult{}, newValidationError("%s yaml is not simulated yet", kind)
	}

	content, err := yaml.Marshal(object)
	if err != nil {
		return ResourceTextResult{}, fmt.Errorf("marshal simulation %s %s/%s yaml: %w", strings.ToLower(kind), namespace, name, err)
	}
	sanitized, err := sanitizeManifestYAML(content)
	if err != nil {
		return ResourceTextResult{}, fmt.Errorf("sanitize simulation %s %s/%s yaml: %w", strings.ToLower(kind), namespace, name, err)
	}
...
func (s *ClusterService) applySimulationResourceYAML(ctx context.Context, kind string, namespace string, name string, content string) (WorkloadActionResult, error) {
	switch strings.ToLower(kind) {
	case "deployment":
		var item appsv1.Deployment
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
		...
		existing, err := s.client.Kubernetes.AppsV1().Deployments(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("get simulation deployment %s/%s before apply: %w", namespace, name, err)
		}
		item.Namespace = namespace
		item.Name = name
		item.UID = existing.UID
		item.ResourceVersion = existing.ResourceVersion
		if item.CreationTimestamp.IsZero() {
			item.CreationTimestamp = existing.CreationTimestamp
		}
		if item.Spec.Replicas == nil {
			item.Spec.Replicas = int32Ptr(desiredReplicas(existing.Spec.Replicas))
		}
		if item.Spec.Selector == nil || len(item.Spec.Selector.MatchLabels) == 0 {
			return WorkloadActionResult{}, newValidationError("deployment spec.selector.matchLabels is required in simulation")
		}
		...
		for key, value := range item.Spec.Selector.MatchLabels {
			if _, ok := item.Spec.Template.Labels[key]; !ok {
				item.Spec.Template.Labels[key] = value
			}
		}

		updated, err := s.client.Kubernetes.AppsV1().Deployments(namespace).Update(ctx, &item, metav1.UpdateOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("apply simulation deployment %s/%s yaml: %w", namespace, name, err)
		}
		if err := s.rollSimulationDeploymentPods(ctx, *updated, "YAMLApply"); err != nil {
			return WorkloadActionResult{}, err
		}
	case "pod":
		var item corev1.Pod
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
			return WorkloadActionResult{}, newValidationError("parse pod yaml: %v", err)
		}
		existing, err := s.client.Kubernetes.CoreV1().Pods(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("get simulation pod %s/%s before apply: %w", namespace, name, err)
		}
		item.Namespace = namespace
		item.Name = name
		item.UID = existing.UID
		...
		if _, err := s.client.Kubernetes.CoreV1().Pods(namespace).Update(ctx, &item, metav1.UpdateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("apply simulation pod %s/%s yaml: %w", namespace, name, err)
		}
	case "service":
		var item corev1.Service
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
		...
		existing, err := s.client.Kubernetes.CoreV1().Services(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("get simulation service %s/%s before apply: %w", namespace, name, err)
		}
		item.Namespace = namespace
		item.Name = name
		item.UID = existing.UID
		...
		if item.Spec.Type == "" {
			item.Spec.Type = existing.Spec.Type
		}
		if _, err := s.client.Kubernetes.CoreV1().Services(namespace).Update(ctx, &item, metav1.UpdateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("apply simulation service %s/%s yaml: %w", namespace, name, err)
		}
		if err := s.reconcileSimulationServiceEndpoints(ctx, namespace, name); err != nil {
			return WorkloadActionResult{}, err
		}
	case "persistentvolumeclaim":
		...
		existing, err := s.client.Kubernetes.CoreV1().PersistentVolumeClaims(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("get simulation persistentvolumeclaim %s/%s before apply: %w", namespace, name, err)
		}
		item.Namespace = namespace
		item.Name = name
		item.UID = existing.UID
		...
		normalizeSimulationPVCStatus(&item)
		if _, err := s.client.Kubernetes.CoreV1().PersistentVolumeClaims(namespace).Update(ctx, &item, metav1.UpdateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("apply simulation persistentvolumeclaim %s/%s yaml: %w", namespace, name, err)
		}
	case "configmap":
		var item corev1.ConfigMap
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
		...
		existing, err := s.client.Kubernetes.CoreV1().ConfigMaps(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("get simulation configmap %s/%s before apply: %w", namespace, name, err)
		}
		item.Namespace = namespace
		item.Name = name
		item.UID = existing.UID
		...
		if _, err := s.client.Kubernetes.CoreV1().ConfigMaps(namespace).Update(ctx, &item, metav1.UpdateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("apply simulation configmap %s/%s yaml: %w", namespace, name, err)
		}
	case "secret":
		var item corev1.Secret
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
		...
		existing, err := s.client.Kubernetes.CoreV1().Secrets(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("get simulation secret %s/%s before apply: %w", namespace, name, err)
		}
		item.Namespace = namespace
		item.Name = name
		item.UID = existing.UID
		...
		if _, err := s.client.Kubernetes.CoreV1().Secrets(namespace).Update(ctx, &item, metav1.UpdateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("apply simulation secret %s/%s yaml: %w", namespace, name, err)
		}
	case "ingress":
		var item networkingv1.Ingress
		if err := yaml.Unmarshal([]byte(content), &item); err != nil {
		...
		existing, err := s.client.Kubernetes.NetworkingV1().Ingresses(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("get simulation ingress %s/%s before apply: %w", namespace, name, err)
		}
		item.Namespace = namespace
		item.Name = name
		item.UID = existing.UID
		...
		if _, err := s.client.Kubernetes.NetworkingV1().Ingresses(namespace).Update(ctx, &item, metav1.UpdateOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("apply simulation ingress %s/%s yaml: %w", namespace, name, err)
		}
	default:
		return WorkloadActionResult{}, newValidationError("%s apply is not simulated yet", kind)
	}
...
func (s *ClusterService) ListDeployments(ctx context.Context, namespace string) ([]DeploymentItem, error) {
	...
	if s.simulationMode() {
		s.fillSimulationPodMetrics(pods.Items, metricsByPod)
	}
...
func (s *ClusterService) ScaleDeployment(
	...
	if s.simulationMode() {
		if err := s.reconcileSimulationDeployment(ctx, namespace, name); err != nil {
			return WorkloadActionResult{}, err
		}
	}
...
func (s *ClusterService) RestartDeployment(
	...
	if s.simulationMode() {
		if err := s.restartSimulationDeploymentPods(ctx, namespace, name); err != nil {
			return WorkloadActionResult{}, err
		}
	}
...
func (s *ClusterService) deleteSimulationResource(
	ctx context.Context,
	kind string,
	namespace string,
	name string,
	...
	switch kind {
	case "Deployment":
		deployment, err := s.client.Kubernetes.AppsV1().Deployments(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("get deployment %s/%s before delete: %w", namespace, name, err)
		}
		selector, err := metav1.LabelSelectorAsSelector(deployment.Spec.Selector)
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("build deployment selector for %s/%s: %w", namespace, name, err)
		}
		if err := s.client.Kubernetes.AppsV1().Deployments(namespace).Delete(ctx, name, metav1.DeleteOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("delete deployment %s/%s: %w", namespace, name, err)
		}
		pods, err := s.client.Kubernetes.CoreV1().Pods(namespace).List(ctx, metav1.ListOptions{})
		if err != nil {
			return WorkloadActionResult{}, fmt.Errorf("list pods for deleted deployment %s/%s: %w", namespace, name, err)
		}
		for _, pod := range pods.Items {
			if selector.Matches(labels.Set(pod.Labels)) {
				_ = s.client.Kubernetes.CoreV1().Pods(namespace).Delete(ctx, pod.Name, metav1.DeleteOptions{})
			}
		}
	case "Service":
		if err := s.client.Kubernetes.CoreV1().Services(namespace).Delete(ctx, name, metav1.DeleteOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("delete service %s/%s: %w", namespace, name, err)
		}
	case "ConfigMap":
		if err := s.client.Kubernetes.CoreV1().ConfigMaps(namespace).Delete(ctx, name, metav1.DeleteOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("delete configmap %s/%s: %w", namespace, name, err)
		}
	case "Secret":
		if err := s.client.Kubernetes.CoreV1().Secrets(namespace).Delete(ctx, name, metav1.DeleteOptions{}); err != nil {
			return WorkloadActionResult{}, fmt.Errorf("delete secret %s/%s: %w", namespace, name, err)
		}
	default:
		return WorkloadActionResult{}, newValidationError("%s delete is not simulated yet", kind)
	}
...
func (s *ClusterService) reconcileSimulationDeployment(ctx context.Context, namespace string, name string) error {
	deployment, err := s.client.Kubernetes.AppsV1().Deployments(namespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("get simulation deployment %s/%s: %w", namespace, name, err)
	}
	selector, err := metav1.LabelSelectorAsSelector(deployment.Spec.Selector)
	if err != nil {
		return fmt.Errorf("build simulation deployment selector for %s/%s: %w", namespace, name, err)
	}

	pods, err := s.client.Kubernetes.CoreV1().Pods(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("list simulation pods for deployment %s/%s: %w", namespace, name, err)
	}
	...
	for int32(len(matched)) > desired {
		last := matched[len(matched)-1]
		if err := s.client.Kubernetes.CoreV1().Pods(namespace).Delete(ctx, last.Name, metav1.DeleteOptions{}); err != nil {
			return fmt.Errorf("delete extra simulation pod %s/%s: %w", namespace, last.Name, err)
		}
		_ = s.createSimulationEvent(ctx, namespace, "Pod", last.Name, corev1.EventTypeNormal, "Killing", fmt.Sprintf("Stopping container for scaled down pod %s", last.Name), 1)
		matched = matched[:len(matched)-1]
	}
	for int32(len(matched)) < desired {
		pod := s.simulationPodForDeployment(ctx, *deployment, len(matched)+1)
		created, err := s.client.Kubernetes.CoreV1().Pods(namespace).Create(ctx, pod, metav1.CreateOptions{})
		if err != nil {
			return fmt.Errorf("create simulation pod for deployment %s/%s: %w", namespace, name, err)
		}
		_ = s.createSimulationPodLifecycleEvents(ctx, created)
		matched = append(matched, *created)
	}

	ready := int32(0)
	for _, pod := range matched {
		if simulationPodReady(pod) {
			ready++
		}
	}
	deployment.Status.Replicas = int32(len(matched))
	deployment.Status.UpdatedReplicas = int32(len(matched))
	deployment.Status.ReadyReplicas = ready
	deployment.Status.AvailableReplicas = ready
	deployment.Status.UnavailableReplicas = int32(len(matched)) - ready
	if _, err := s.client.Kubernetes.AppsV1().Deployments(namespace).Update(ctx, deployment, metav1.UpdateOptions{}); err != nil {
		return fmt.Errorf("update simulation deployment status for %s/%s: %w", namespace, name, err)
	}

	return nil
}

func (s *ClusterService) restartSimulationDeploymentPods(ctx context.Context, namespace string, name string) error {
	deployment, err := s.client.Kubernetes.AppsV1().Deployments(namespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("get simulation deployment %s/%s before restart: %w", namespace, name, err)
	}
	return s.rollSimulationDeploymentPods(ctx, *deployment, "Restarted")
}

func (s *ClusterService) rollSimulationDeploymentPods(ctx context.Context, deployment appsv1.Deployment, reason string) error {
	namespace := deployment.Namespace
	name := deployment.Name
	selector, err := metav1.LabelSelectorAsSelector(deployment.Spec.Selector)
	if err != nil {
		return fmt.Errorf("build simulation deployment selector for rollout %s/%s: %w", namespace, name, err)
	}
	pods, err := s.client.Kubernetes.CoreV1().Pods(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("list simulation pods for rollout %s/%s: %w", namespace, name, err)
	}
	for _, pod := range pods.Items {
		if selector.Matches(labels.Set(pod.Labels)) {
			_ = s.client.Kubernetes.CoreV1().Pods(namespace).Delete(ctx, pod.Name, metav1.DeleteOptions{})
			_ = s.createSimulationEvent(ctx, namespace, "Pod", pod.Name, corev1.EventTypeNormal, "Killing", fmt.Sprintf("Stopping container for rolled pod %s", pod.Name), 1)
		}
	}
	_ = s.createSimulationEvent(ctx, namespace, "Deployment", name, corev1.EventTypeNormal, reason, fmt.Sprintf("Deployment %s/%s triggered a simulated rollout", namespace, name), 1)

	return s.reconcileSimulationDeployment(ctx, namespace, name)
}

func (s *ClusterService) createSimulationPodLifecycleEvents(ctx context.Context, pod *corev1.Pod) error {
	if pod == nil {
		return nil
	}
	if pod.Status.Reason == "Unschedulable" || pod.Spec.NodeName == "" {
		message := "0/2 nodes are available: pod did not fit any simulated node"
		for _, condition := range pod.Status.Conditions {
			if condition.Type == corev1.PodScheduled && strings.TrimSpace(condition.Message) != "" {
				message = condition.Message
				break
			}
		}
		_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeWarning, "FailedScheduling", message, 4)
		return nil
	}
	_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeNormal, "Scheduled", fmt.Sprintf("Successfully assigned %s/%s to %s", pod.Namespace, pod.Name, pod.Spec.NodeName), 1)
	for _, status := range pod.Status.ContainerStatuses {
		if status.Ready {
			_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeNormal, "Pulling", fmt.Sprintf("Pulling image %q", status.Image), 1)
			_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeNormal, "Pulled", fmt.Sprintf("Container image %q already present on machine", status.Image), 1)
			_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeNormal, "Created", fmt.Sprintf("Created container %s", status.Name), 1)
			_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeNormal, "Started", fmt.Sprintf("Started container %s", status.Name), 1)
			continue
		}
		if status.State.Waiting != nil {
			...
			case "ImagePullBackOff":
				_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeNormal, "Pulling", fmt.Sprintf("Pulling image %q", status.Image), 2)
				_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeWarning, "Failed", fmt.Sprintf("Failed to pull image %q: rpc error: code = NotFound desc = failed to resolve reference", status.Image), 2)
				_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeWarning, "ErrImagePull", fmt.Sprintf("Error: ErrImagePull for image %q", status.Image), 2)
				message = fmt.Sprintf("Back-off pulling image %q", status.Image)
			case "CrashLoopBackOff":
				_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeWarning, "BackOff", fmt.Sprintf("Back-off restarting failed container %s in pod %s", status.Name, pod.Name), 8)
			case "CreateContainerConfigError":
				_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeWarning, "Failed", fmt.Sprintf("Error: couldn't find key or referenced object: %s", message), 3)
			}
			_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeWarning, reason, message, 3)
			continue
		}
		if status.State.Running != nil {
			_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeNormal, "Pulled", fmt.Sprintf("Container image %q already present on machine", status.Image), 1)
			_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeNormal, "Started", fmt.Sprintf("Started container %s", status.Name), 1)
			_ = s.createSimulationEvent(ctx, pod.Namespace, "Pod", pod.Name, corev1.EventTypeWarning, "Unhealthy", fmt.Sprintf("Readiness probe failed: HTTP probe failed with statuscode: 500 for container %s", status.Name), 5)
		}
	}
	return nil
}

func (s *ClusterService) createSimulationEvent(ctx context.Context, namespace string, kind string, name string, eventType string, reason string, message string, count int32) error {
	now := metav1.NewTime(time.Now())
	event := &corev1.Event{
		ObjectMeta: metav1.ObjectMeta{
			Name:              fmt.Sprintf("%s-%s-%s-%d", strings.ToLower(kind), name, strings.ToLower(reason), time.Now().UnixNano()),
			Namespace:         namespace,
	...
	_, err := s.client.Kubernetes.CoreV1().Events(namespace).Create(ctx, event, metav1.CreateOptions{})
	return err
}

func (s *ClusterService) fillSimulationPodMetrics(pods []corev1.Pod, metricsByPod map[string]metricsv1beta1.PodMetrics) {
	for _, pod := range pods {
		key := podMetricsKey(pod.Namespace, pod.Name)
		if _, exists := metricsByPod[key]; exists || pod.Status.Phase != corev1.PodRunning {
			continue
		}
		containers := make([]metricsv1beta1.ContainerMetrics, 0, len(pod.Spec.Containers))
		for index, container := range pod.Spec.Containers {
			containers = append(containers, metricsv1beta1.ContainerMetrics{
				Name: container.Name,
				Usage: corev1.ResourceList{
					corev1.ResourceCPU:    resource.MustParse(fmt.Sprintf("%dm", 35+len(container.Image)%70+index*8)),
					corev1.ResourceMemory: resource.MustParse(fmt.Sprintf("%dMi", 48+len(container.Name)*6+index*12)),
				},
			})
		}
		if len(containers) == 0 {
		...
		metricsByPod[key] = metricsv1beta1.PodMetrics{
			ObjectMeta: metav1.ObjectMeta{Name: pod.Name, Namespace: pod.Namespace, CreationTimestamp: metav1.NewTime(time.Now())},
			Containers: containers,
		}
	}
}

func (s *ClusterService) reconcileSimulationServiceEndpoints(ctx context.Context, namespace string, name string) error {
	services, err := s.client.Kubernetes.CoreV1().Services(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("list simulation services for endpoints: %w", err)
	}
	pods, err := s.client.Kubernetes.CoreV1().Pods(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("list simulation pods for endpoints: %w", err)
	}
	...
	for _, service := range services.Items {
		...
		endpoints := simulationEndpointsForService(service, pods.Items)
		existing, err := s.client.Kubernetes.CoreV1().Endpoints(service.Namespace).Get(ctx, service.Name, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			if _, err := s.client.Kubernetes.CoreV1().Endpoints(service.Namespace).Create(ctx, endpoints, metav1.CreateOptions{}); err != nil {
				return fmt.Errorf("create simulation endpoints %s/%s: %w", service.Namespace, service.Name, err)
			}
			continue
		}
		if err != nil {
			return fmt.Errorf("get simulation endpoints %s/%s: %w", service.Namespace, service.Name, err)
		}
		endpoints.ResourceVersion = existing.ResourceVersion
		if _, err := s.client.Kubernetes.CoreV1().Endpoints(service.Namespace).Update(ctx, endpoints, metav1.UpdateOptions{}); err != nil {
			return fmt.Errorf("update simulation endpoints %s/%s: %w", service.Namespace, service.Name, err)
		}
	}
...
func simulationEndpointsForService(service corev1.Service, pods []corev1.Pod) *corev1.Endpoints {
	...
	for _, port := range service.Spec.Ports {
		...
		if port.TargetPort.Type == intstr.Int && port.TargetPort.IntVal > 0 {
			endpointPort = port.TargetPort.IntVal
		}
...
func (s *ClusterService) simulationPodDeploymentOwner(ctx context.Context, pod *corev1.Pod) string {
	if pod == nil {
		return ""
	}
	for _, owner := range pod.OwnerReferences {
		...
		replicaSet, err := s.client.Kubernetes.AppsV1().ReplicaSets(pod.Namespace).Get(ctx, owner.Name, metav1.GetOptions{})
		if err != nil {
			continue
		}
...
func (s *ClusterService) simulationPodForDeployment(ctx context.Context, deployment appsv1.Deployment, ordinal int) *corev1.Pod {
	createdAt := metav1.NewTime(time.Now())
	labels := make(map[string]string, len(deployment.Spec.Template.Labels)+1)
	...
	crashLoopFailure := false
	probeFailure := false
	oomKilled := false
	configErrorReason := ""
	configErrorMessage := ""
	for _, container := range deployment.Spec.Template.Spec.Containers {
		if simulationImagePullFails(container.Image) {
			imagePullFailure = true
			break
		}
		if reason, message := s.simulationContainerConfigError(ctx, deployment.Namespace, container); reason != "" {
			configErrorReason = reason
			configErrorMessage = message
		}
		if simulationContainerOOMKilled(container) {
			oomKilled = true
		}
		if simulationContainerCrashes(container) {
			crashLoopFailure = true
		}
		if simulationContainerProbeFails(container) {
			probeFailure = true
		}
	}
	unschedulableReason := simulationUnschedulableReason(deployment.Spec.Template.Spec.Containers)

	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:              fmt.Sprintf("%s-sim-%s-%d", deployment.Name, strconv.FormatInt(time.Now().UnixNano(), 36), ordinal),
			Namespace:         deployment.Namespace,
			UID:               types.UID(fmt.Sprintf("pod-%s-%d", deployment.Name, time.Now().UnixNano())),
	...
	if pod.Spec.NodeName == "" {
		pod.Spec.NodeName = "sim-worker-1"
	}
	...
	if unschedulableReason != "" {
		...
		pod.Status = corev1.PodStatus{
			...
			Conditions: []corev1.PodCondition{
				{Type: corev1.PodScheduled, Status: corev1.ConditionFalse, Reason: "Unschedulable", Message: unschedulableReason},
				{Type: corev1.PodReady, Status: corev1.ConditionFalse, Reason: "PodScheduledFalse"},
				{Type: corev1.ContainersReady, Status: corev1.ConditionFalse, Reason: "PodScheduledFalse"},
			},
			ContainerStatuses: simulationWaitingContainerStatuses(pod.Spec.Containers, "ContainerCreating", "waiting for pod to be scheduled"),
		}
		return pod
	}
	...
	if imagePullFailure {
		pod.Status = corev1.PodStatus{
			...
			ContainerStatuses: simulationContainerStatuses(pod.Spec.Containers, false, createdAt),
		}
		return pod
	}
	...
	if configErrorReason != "" {
		pod.Status = corev1.PodStatus{
			...
			ContainerStatuses: simulationWaitingContainerStatuses(pod.Spec.Containers, configErrorReason, configErrorMessage),
		}
		return pod
	}
	...
	if crashLoopFailure {
		pod.Status = corev1.PodStatus{
			...
			ContainerStatuses: simulationCrashLoopContainerStatuses(pod.Spec.Containers, createdAt),
		}
		return pod
	}
	...
	if oomKilled {
		pod.Status = corev1.PodStatus{
			...
			ContainerStatuses: simulationOOMKilledContainerStatuses(pod.Spec.Containers, createdAt),
		}
		return pod
	}
	...
	if probeFailure {
		pod.Status = corev1.PodStatus{
			Phase:    corev1.PodRunning,
			PodIP:    fmt.Sprintf("10.244.2.%d", 30+ordinal),
			QOSClass: corev1.PodQOSBurstable,
			Conditions: []corev1.PodCondition{
				{Type: corev1.PodReady, Status: corev1.ConditionFalse, Reason: "ContainersNotReady"},
				{Type: corev1.ContainersReady, Status: corev1.ConditionFalse, Reason: "ContainersNotReady"},
			},
			ContainerStatuses: simulationProbeFailureContainerStatuses(pod.Spec.Containers, createdAt),
		}
		return pod
	}
	...
	pod.Status = corev1.PodStatus{
		...
		ContainerStatuses: simulationContainerStatuses(pod.Spec.Containers, true, createdAt),
	}

	return pod
}

func simulationContainerStatuses(containers []corev1.Container, running bool, startedAt metav1.Time) []corev1.ContainerStatus {
	...
	for _, container := range containers {
		...
		if running {
			status.State = corev1.ContainerState{Running: &corev1.ContainerStateRunning{StartedAt: startedAt}}
		} else {
			status.State = corev1.ContainerState{Waiting: &corev1.ContainerStateWaiting{
				Reason:  "ImagePullBackOff",
				Message: fmt.Sprintf("Back-off pulling image %q", container.Image),
			}}
		}
...
func simulationWaitingContainerStatuses(containers []corev1.Container, reason string, message string) []corev1.ContainerStatus {
	statuses := make([]corev1.ContainerStatus, 0, len(containers))
	for _, container := range containers {
...
func simulationCrashLoopContainerStatuses(containers []corev1.Container, startedAt metav1.Time) []corev1.ContainerStatus {
	statuses := make([]corev1.ContainerStatus, 0, len(containers))
	for _, container := range containers {
		statuses = append(statuses, corev1.ContainerStatus{
			Name:         container.Name,
			...
			State: corev1.ContainerState{Waiting: &corev1.ContainerStateWaiting{
				...
				Message: fmt.Sprintf("back-off restarting failed container=%s", container.Name),
			}},
			LastTerminationState: corev1.ContainerState{Terminated: &corev1.ContainerStateTerminated{
				...
				StartedAt:  startedAt,
				FinishedAt: metav1.NewTime(startedAt.Time.Add(4 * time.Second)),
			}},
		})
	}
	return statuses
...
func simulationOOMKilledContainerStatuses(containers []corev1.Container, startedAt metav1.Time) []corev1.ContainerStatus {
	statuses := make([]corev1.ContainerStatus, 0, len(containers))
	for _, container := range containers {
		statuses = append(statuses, corev1.ContainerStatus{
			Name:         container.Name,
			...
			State: corev1.ContainerState{Waiting: &corev1.ContainerStateWaiting{
				Reason:  "CrashLoopBackOff",
				Message: fmt.Sprintf("back-off restarting failed container=%s after OOMKilled", container.Name),
			}},
			LastTerminationState: corev1.ContainerState{Terminated: &corev1.ContainerStateTerminated{
				ExitCode:   137,
				Reason:     "OOMKilled",
				...
				StartedAt:  startedAt,
				FinishedAt: metav1.NewTime(startedAt.Time.Add(30 * time.Second)),
			}},
		})
	}
	return statuses
...
func simulationProbeFailureContainerStatuses(containers []corev1.Container, startedAt metav1.Time) []corev1.ContainerStatus {
	statuses := make([]corev1.ContainerStatus, 0, len(containers))
	for _, container := range containers {
		statuses = append(statuses, corev1.ContainerStatus{
			Name:         container.Name,
...
func simulationImagePullFails(image string) bool {
	...
	return strings.HasSuffix(value, ":missing") || strings.HasSuffix(value, ":404")
}

func (s *ClusterService) simulationContainerConfigError(ctx context.Context, namespace string, container corev1.Container) (string, string) {
	for _, from := range container.EnvFrom {
		if from.ConfigMapRef != nil && strings.TrimSpace(from.ConfigMapRef.Name) != "" {
			name := strings.TrimSpace(from.ConfigMapRef.Name)
			if _, err := s.client.Kubernetes.CoreV1().ConfigMaps(namespace).Get(ctx, name, metav1.GetOptions{}); apierrors.IsNotFound(err) && !optionalBoolValue(from.ConfigMapRef.Optional) {
				return "CreateContainerConfigError", fmt.Sprintf("configmap %q not found", name)
			}
		}
		if from.SecretRef != nil && strings.TrimSpace(from.SecretRef.Name) != "" {
			name := strings.TrimSpace(from.SecretRef.Name)
			if _, err := s.client.Kubernetes.CoreV1().Secrets(namespace).Get(ctx, name, metav1.GetOptions{}); apierrors.IsNotFound(err) && !optionalBoolValue(from.SecretRef.Optional) {
				return "CreateContainerConfigError", fmt.Sprintf("secret %q not found", name)
			}
		}
	}
	...
	for _, env := range container.Env {
		...
		if ref := env.ValueFrom.ConfigMapKeyRef; ref != nil && strings.TrimSpace(ref.Name) != "" {
			name := strings.TrimSpace(ref.Name)
			if _, err := s.client.Kubernetes.CoreV1().ConfigMaps(namespace).Get(ctx, name, metav1.GetOptions{}); apierrors.IsNotFound(err) && !optionalBoolValue(ref.Optional) {
				return "CreateContainerConfigError", fmt.Sprintf("configmap %q not found while resolving env %s", name, env.Name)
			}
		}
		if ref := env.ValueFrom.SecretKeyRef; ref != nil && strings.TrimSpace(ref.Name) != "" {
			name := strings.TrimSpace(ref.Name)
			if _, err := s.client.Kubernetes.CoreV1().Secrets(namespace).Get(ctx, name, metav1.GetOptions{}); apierrors.IsNotFound(err) && !optionalBoolValue(ref.Optional) {
				return "CreateContainerConfigError", fmt.Sprintf("secret %q not found while resolving env %s", name, env.Name)
			}
		}
	}
...
func simulationUnschedulableReason(containers []corev1.Container) string {
	totalCPU := int64(0)
	totalMemory := int64(0)
	for _, container := range containers {
		if quantity, ok := container.Resources.Requests[corev1.ResourceCPU]; ok {
			totalCPU += quantity.MilliValue()
		}
		if quantity, ok := container.Resources.Requests[corev1.ResourceMemory]; ok {
			totalMemory += quantity.Value()
		}
	}
	if totalCPU > 12000 {
		return fmt.Sprintf("0/2 nodes are available: insufficient cpu. requested %dm, cluster allocatable around 12000m", totalCPU)
	}
	if totalMemory > 24*1024*1024*1024 {
		return fmt.Sprintf("0/2 nodes are available: insufficient memory. requested %s, cluster allocatable around 24Gi", resource.NewQuantity(totalMemory, resource.BinarySI).String())
	}
	return ""
}

func simulationContainerOOMKilled(container corev1.Container) bool {
	joined := strings.ToLower(strings.Join(append(append([]string{}, container.Command...), container.Args...), " "))
	if strings.Contains(joined, "simulate-oom") || strings.Contains(joined, "oom") {
		return true
	}
	if limit, ok := container.Resources.Limits[corev1.ResourceMemory]; ok {
		return limit.Cmp(resource.MustParse("64Mi")) <= 0
	}
	return false
}

func simulationContainerCrashes(container corev1.Container) bool {
	if strings.Contains(strings.ToLower(container.Image), "crash") {
		return true
	}
	joined := strings.ToLower(strings.Join(append(append([]string{}, container.Command...), container.Args...), " "))
	return strings.Contains(joined, "exit 1") || strings.Contains(joined, "panic") || strings.Contains(joined, "throw")
}

func simulationContainerProbeFails(container corev1.Container) bool {
	if container.ReadinessProbe != nil && container.ReadinessProbe.HTTPGet != nil {
		path := strings.ToLower(strings.TrimSpace(container.ReadinessProbe.HTTPGet.Path))
		return strings.Contains(path, "fail") || strings.Contains(path, "bad") || strings.Contains(path, "404")
	}
	return false
}

func normalizeSimulationPVCStatus(item *corev1.PersistentVolumeClaim) {
	if item == nil {
		return
	}
	storageClass := ""
	if item.Spec.StorageClassName != nil {
		storageClass = strings.TrimSpace(*item.Spec.StorageClassName)
	}
	requested := resource.MustParse("1Gi")
	if quantity, ok := item.Spec.Resources.Requests[corev1.ResourceStorage]; ok {
		requested = quantity
	}
	if storageClass == "" || storageClass == "local-path" {
		item.Status.Phase = corev1.ClaimBound
		item.Status.Capacity = corev1.ResourceList{corev1.ResourceStorage: requested}
		if item.Spec.VolumeName == "" {
			item.Spec.VolumeName = fmt.Sprintf("pvc-%s", item.Name)
		}
		return
	}
	item.Status.Phase = corev1.ClaimPending
	item.Status.Capacity = nil
	item.Spec.VolumeName = ""
}

func simulationPodLogs(pod corev1.Pod, container string, tailLines int64) string {
	now := time.Now()
	selected := corev1.Container{}
	...
	if selected.Name == "" && len(pod.Spec.Containers) > 0 {
		selected = pod.Spec.Containers[0]
	}

	lines := []string{
		fmt.Sprintf("%s INFO  [%s/%s] boot sequence started, image=%s", now.Add(-45*time.Second).Format(time.RFC3339), pod.Namespace, pod.Name, selected.Image),
		fmt.Sprintf("%s INFO  [%s] loading config maps and projected service account token", now.Add(-38*time.Second).Format(time.RFC3339), container),
		fmt.Sprintf("%s INFO  [%s] listening on 0.0.0.0:%s", now.Add(-31*time.Second).Format(time.RFC3339), container, simulationContainerPort(selected)),
		fmt.Sprintf("%s INFO  [%s] readiness probe passed", now.Add(-20*time.Second).Format(time.RFC3339), container),
		fmt.Sprintf("%s INFO  [%s] request completed method=GET path=/healthz status=200 latency=3ms", now.Add(-9*time.Second).Format(time.RFC3339), container),
	}
	...
	for _, status := range pod.Status.ContainerStatuses {
		...
		if status.State.Running != nil && !status.Ready {
			lines = []string{
				fmt.Sprintf("%s INFO  [%s/%s] application started, image=%s", now.Add(-45*time.Second).Format(time.RFC3339), pod.Namespace, pod.Name, status.Image),
				fmt.Sprintf("%s WARN  [%s] readiness probe returned HTTP 500 for /fail", now.Add(-33*time.Second).Format(time.RFC3339), container),
				fmt.Sprintf("%s WARN  [%s] pod is running but not receiving Service traffic", now.Add(-18*time.Second).Format(time.RFC3339), container),
			}
		}
		if status.State.Waiting != nil {
			if status.State.Waiting.Reason == "CrashLoopBackOff" {
				if status.LastTerminationState.Terminated != nil && status.LastTerminationState.Terminated.Reason == "OOMKilled" {
					lines = []string{
						fmt.Sprintf("%s INFO  [%s/%s] starting memory intensive job image=%s", now.Add(-45*time.Second).Format(time.RFC3339), pod.Namespace, pod.Name, status.Image),
						fmt.Sprintf("%s WARN  [%s] memory usage reached configured container limit", now.Add(-34*time.Second).Format(time.RFC3339), container),
						fmt.Sprintf("%s ERROR [%s] terminated by kernel OOM killer exit_code=137 reason=OOMKilled", now.Add(-28*time.Second).Format(time.RFC3339), container),
						fmt.Sprintf("%s WARN  [%s] kubelet will restart the container after backoff", now.Add(-16*time.Second).Format(time.RFC3339), container),
					}
				} else {
					lines = []string{
						fmt.Sprintf("%s INFO  [%s/%s] starting container image=%s", now.Add(-45*time.Second).Format(time.RFC3339), pod.Namespace, pod.Name, status.Image),
						fmt.Sprintf("%s INFO  [%s] starting payment worker", now.Add(-42*time.Second).Format(time.RFC3339), container),
						fmt.Sprintf("%s ERROR [%s] process exited with status 1", now.Add(-38*time.Second).Format(time.RFC3339), container),
						fmt.Sprintf("%s WARN  [%s] kubelet will restart the container after backoff", now.Add(-21*time.Second).Format(time.RFC3339), container),
					}
				}
			} else {
				lines = []string{
					fmt.Sprintf("%s WARN  [%s/%s] waiting for container image=%s", now.Add(-45*time.Second).Format(time.RFC3339), pod.Namespace, pod.Name, status.Image),
					fmt.Sprintf("%s ERROR [%s] %s: %s", now.Add(-36*time.Second).Format(time.RFC3339), container, status.State.Waiting.Reason, status.State.Waiting.Message),
					fmt.Sprintf("%s WARN  [%s] kubelet will retry image pull with exponential backoff", now.Add(-21*time.Second).Format(time.RFC3339), container),
				}
			}
		}
		break
...
func simulationPodDescribe(pod corev1.Pod, events []PodEventItem) string {
	var builder strings.Builder
	builder.WriteString(fmt.Sprintf("Name:             %s\n", pod.Name))
	builder.WriteString(fmt.Sprintf("Namespace:        %s\n", pod.Namespace))
	builder.WriteString("Priority:         0\n")
	builder.WriteString("Service Account:  default\n")
	builder.WriteString(fmt.Sprintf("Node:             %s\n", defaultString(pod.Spec.NodeName, "<none>")))
	builder.WriteString(fmt.Sprintf("Start Time:       %s\n", pod.CreationTimestamp.Time.Format("2006-01-02 15:04:05")))
	builder.WriteString(fmt.Sprintf("Labels:           %s\n", defaultString(strings.Join(labelPairs(pod.Labels), ","), "<none>")))
	builder.WriteString(fmt.Sprintf("Status:           %s\n", podListStatus(pod)))
	builder.WriteString(fmt.Sprintf("IP:               %s\n", defaultString(pod.Status.PodIP, "<none>")))
	builder.WriteString(fmt.Sprintf("Controlled By:    %s\n", simulationOwnerSummary(pod.OwnerReferences)))
	builder.WriteString("Containers:\n")
	for _, container := range pod.Spec.Containers {
		builder.WriteString(fmt.Sprintf("  %s:\n", container.Name))
		builder.WriteString(fmt.Sprintf("    Image:          %s\n", container.Image))
		builder.WriteString(fmt.Sprintf("    Port:           %s/TCP\n", simulationContainerPort(container)))
		builder.WriteString(fmt.Sprintf("    Host Port:      0/TCP\n"))
		builder.WriteString(fmt.Sprintf("    Command:        %s\n", defaultString(strings.Join(container.Command, " "), "<none>")))
		builder.WriteString(fmt.Sprintf("    Args:           %s\n", defaultString(strings.Join(container.Args, " "), "<none>")))
		for _, status := range pod.Status.ContainerStatuses {
			if status.Name != container.Name {
				continue
			}
			builder.WriteString(fmt.Sprintf("    Ready:          %t\n", status.Ready))
			builder.WriteString(fmt.Sprintf("    Restart Count:  %d\n", status.RestartCount))
			if status.State.Waiting != nil {
				builder.WriteString("    State:          Waiting\n")
				builder.WriteString(fmt.Sprintf("      Reason:       %s\n", status.State.Waiting.Reason))
				builder.WriteString(fmt.Sprintf("      Message:      %s\n", status.State.Waiting.Message))
			}
			if status.State.Running != nil {
				builder.WriteString("    State:          Running\n")
				builder.WriteString(fmt.Sprintf("      Started:      %s\n", status.State.Running.StartedAt.Time.Format("2006-01-02 15:04:05")))
			}
			if status.LastTerminationState.Terminated != nil {
				terminated := status.LastTerminationState.Terminated
				builder.WriteString("    Last State:     Terminated\n")
				builder.WriteString(fmt.Sprintf("      Reason:       %s\n", terminated.Reason))
		...
		for _, envFrom := range container.EnvFrom {
			if envFrom.ConfigMapRef != nil {
				builder.WriteString(fmt.Sprintf("      From ConfigMap: %s\n", envFrom.ConfigMapRef.Name))
			}
			if envFrom.SecretRef != nil {
				builder.WriteString(fmt.Sprintf("      From Secret:    %s\n", envFrom.SecretRef.Name))
			}
		}
		for _, env := range container.Env {
			if env.Value != "" {
			...
			builder.WriteString(fmt.Sprintf("      %s: <set to the key of a referenced resource>\n", env.Name))
		}
		builder.WriteString("    Mounts:\n")
		if len(container.VolumeMounts) == 0 {
			builder.WriteString("      /var/run/secrets/kubernetes.io/serviceaccount from kube-api-access-sim (ro)\n")
		}
		for _, mount := range container.VolumeMounts {
			builder.WriteString(fmt.Sprintf("      %s from %s (%s)\n", mount.MountPath, mount.Name, readOnlyText(mount.ReadOnly)))
		}
	}
	builder.WriteString("Conditions:\n")
	builder.WriteString("  Type              Status  Reason\n")
	for _, condition := range pod.Status.Conditions {
		builder.WriteString(fmt.Sprintf("  %-17s %-7s %s\n", condition.Type, condition.Status, defaultString(condition.Reason, "-")))
	...
	if len(pod.Spec.Volumes) == 0 {
		builder.WriteString("  kube-api-access-sim:\n")
		builder.WriteString("    Type:                    Projected\n")
		builder.WriteString("    TokenExpirationSeconds:  3607\n")
	} else {
		for _, volume := range pod.Spec.Volumes {
			builder.WriteString(fmt.Sprintf("  %s:\n", volume.Name))
			builder.WriteString(fmt.Sprintf("    Type:                    %s\n", simulationVolumeType(volume)))
		}
	}
	builder.WriteString(fmt.Sprintf("QoS Class:         %s\n", defaultString(string(pod.Status.QOSClass), "BestEffort")))
	builder.WriteString("Node-Selectors:    <none>\n")
	builder.WriteString("Tolerations:       node.kubernetes.io/not-ready:NoExecute op=Exists for 300s\n")
	builder.WriteString("                   node.kubernetes.io/unreachable:NoExecute op=Exists for 300s\n")
	builder.WriteString("Events:\n")
	if len(events) == 0 {
		builder.WriteString("  <none>\n")
		return builder.String()
	}
	builder.WriteString("  Type     Reason                Age                 From               Message\n")
	for _, event := range events {
		builder.WriteString(fmt.Sprintf("  %-8s %-21s %-19s kubelet/scheduler  %s", event.Type, event.Reason, event.LastSeen, event.Message))
		if event.Count > 1 {
			builder.WriteString(fmt.Sprintf(" (x%d)", event.Count))
		}
		builder.WriteString("\n")
	}
	return builder.String()
...
func simulationOwnerSummary(owners []metav1.OwnerReference) string {
	if len(owners) == 0 {
		return "<none>"
	}
	owner := owners[0]
...
func simulationVolumeType(volume corev1.Volume) string {
	switch {
...
func simulationContainerPort(container corev1.Container) string {
...
func simulationPodReady(pod corev1.Pod) bool {
	if len(pod.Spec.Containers) == 0 {
		return false
	}
	return podReadyContainerCount(pod) == len(pod.Spec.Containers)
}
