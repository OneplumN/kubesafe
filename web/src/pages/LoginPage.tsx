...
export function LoginPage() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: (result, token) => {
      ...
      void message.success(
        result.authMode === 'simulation'
          ? '已进入 Kubernetes 模拟操作平台'
          : 'ServiceAccount Token 校验通过，已接入真实集群',
      );
    },
  });
  ...
  const handleSimulationEnter = () => {
    loginMutation.mutate('kubesafe-simulation-token');
  };
