...
export type ManifestBatchResult = {
  count: number;
  results: WorkloadActionResult[];
  message: string;
  timestamp: string;
};

export type SimulationResetResult = {
  namespace: string;
  deleted: number;
  resources: string[];
  message: string;
...
export type SimulationScenarioResource = {
  kind: string;
  namespace?: string;
  name: string;
  path: string;
...
export type SimulationScenarioCommand = {
  title: string;
  command: string;
  output: string;
};
...
export async function createManifests(content: string) {
  const { data } = await http.post<Envelope<ManifestBatchResult>>('/manifests/batch', {
    content,
  });
  return data.data;
}

export async function applyManifests(content: string) {
  const { data } = await http.post<Envelope<ManifestBatchResult>>('/manifests/apply', {
    content,
  });
  return data.data;
}

export async function resetSimulationNamespace(namespace: string) {
  const { data } = await http.post<Envelope<SimulationResetResult>>('/simulation/reset', null, {
    params: { namespace },
  });
  return data.data;
}
...
export async function getSimulationScenarios() {
  const { data } = await http.get<Envelope<SimulationScenario[]>>('/simulation/scenarios');
  return data.data;
}

export async function resetSimulationScenario(id: string) {
  const { data } = await http.post<Envelope<SimulationScenario>>(
    `/simulation/scenarios/${encodeURIComponent(id)}/reset`,
  );
  return data.data;
}
