...
```bash
cd server
export KUBESAFE_KUBECONFIG=/path/to/your/dev-kubeconfig
go run ./cmd/kubesafe
```

### 模拟平台模式

无需真实 kubeconfig，可启动内置 Kubernetes 沙盘数据：

```bash
cd server
KUBESAFE_MODE=simulation go run ./cmd/kubesafe
```

前端登录页选择“进入模拟平台”即可接入模拟集群。
...
```bash
./server/dist/release/<package-dir>/kubesafe --version
```
...
```bash
KUBESAFE_UPDATE_ENABLED=true
KUBESAFE_UPDATE_ALLOW_PRERELEASES=true
KUBESAFE_UPDATE_REPOSITORY=OneplumN/kubesafe
KUBESAFE_UPDATE_ALLOWED_SUBJECTS=system:serviceaccount:kube-system:kubesafe-dev
KUBESAFE_UPDATE_GITHUB_TOKEN=<optional-github-token>
KUBESAFE_UPDATE_TARGET_PATH=<optional-installed-binary-path>
```

说明：

- `KUBESAFE_UPDATE_ENABLED`
  - 是否启用在线更新入口
- `KUBESAFE_UPDATE_ALLOW_PRERELEASES`
  - 是否允许检测和安装 `rc / beta / alpha` 预发布版本
- `KUBESAFE_UPDATE_REPOSITORY`
  - GitHub Releases 仓库，默认 `OneplumN/kubesafe`
- `KUBESAFE_UPDATE_ALLOWED_SUBJECTS`
  - 允许执行更新、回滚、重启的 Kubernetes 身份白名单，逗号分隔
- `KUBESAFE_UPDATE_GITHUB_TOKEN`
  - 可选，用于提升 GitHub API 访问稳定性和速率限制配额
- `KUBESAFE_UPDATE_TARGET_PATH`
